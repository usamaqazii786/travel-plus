...
import { AddAboutService, EditAboutService } from '@/utils/Services/AboutServices'
import { validationSchema } from './AddAboutSchema'
...
export default function AddAbout() {
  ...
  const onSubmit = (data) => {
    if (state?.id) {
      EditAboutService(data, navigate, showNotification, setloading, state?.id)
    } else {
      AddAboutService(data, navigate, showNotification, setloading)
    }
  }
