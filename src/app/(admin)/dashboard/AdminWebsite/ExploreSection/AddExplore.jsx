...
import { AddExploreService,EditExploreService } from '@/utils/Services/ExploreServices'
import { validationSchema } from './AddExploreSchema'
...
export default function AddExplore() {
  ...
  const onSubmit = (data) => {
    if (state?.id) {
      EditExploreService(data, navigate, showNotification, setloading, state?.id)
    } else {
      AddExploreService(data, navigate, showNotification, setloading)
    }
  }
