...
const W_nineform = withSwal((props) => {
  const { swal } = props
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const inputValuesExists = inputValues.some((value) => value.trim() !== '')
      const inputValuesemExists = inputValuesemp.some((value) => value.trim() !== '')
      if (inputValuesExists) {
        formDatas.append('ss_number', inputValues)
      }
      if (inputValuesemExists) {
      ...
      await axiosInstance.post('/subadmin/agent/wform', formDatas).then((response) => {
        if (response?.data?.response) {
          showNotification({ message: response?.data?.response, variant: 'success' })
          navigate('/agentdashboard/wnine')
        }
      })
    } catch (error) {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axiosInstance.post(`/admin/agent/wform/${state?.id}`, formDatas).then((response) => {
        if (response?.data?.response) {
          showNotification({ message: response?.data?.response, variant: 'success' })
          navigate('/dashboard/wnine')
        }
      })
    } catch (error) {
      showNotification({ message: error?.response?.data?.message, variant: 'error' })
  ...
  useEffect(() => {
    if (IsnEwUser==='false') {
      const text = `Please fill out and submit your ${user?.location_status === 'Yes' ? 'W-9' : 'W-8'} form. After submission, log out and log back in to access CRM tools.`
      swal
        .fire({
          title: 'Action Required:',
          text: `Welcome to Travel Plus /${text}`,
  ...
  return (
    <div className="container-fluid bg-white">
      ...
      <div className="row" style={{ padding: '0' }}>
        <div className="col-md-12" style={{ padding: '0', fontSize: '15px', borderBottom: '2px solid black', borderColor: 'black' }}>
          <span style={{ fontWeight: '900' }}> Before you begin.</span> For guidance related to the purpose of Form W-9, see Purpose of Form, below
        </div>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', flexDirection: 'column', borderBottom: '1px solid black' }}>
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <div>
                {' '}
                Name of entity/individual. An entry is required. (For a sole proprietor or disregarded entity, enter the owner’s name on line 1, and
          ...
          <div style={{ display: 'flex' }}>
            <div
              ...
              <div style={{ display: 'flex', flexDirection: 'column', borderBottom: '1px solid black', gap: '10px' }}>
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <div>
                    Check the appropriate box for federal tax classification of the entity/individual whose name is entered on line 1. Check only one
                    of the following seven boxes.{' '}
                  </div>
                </div>
                <div style={{ display: 'flex', padding: '0 15px', gap: '30px' }}>
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <input type="checkbox" name="individual_sole_proprietor" checked={formData.individual_sole_proprietor} onChange={handleChange} />
                    Individual/sole proprietor
                  </div>
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <input type="checkbox" name="c_corporation" checked={formData.c_corporation} onChange={handleChange} />C corporation
                  </div>
                  <div style={{ display: 'flex', gap: '10px' }}>
                    <input type="checkbox" name="a_corporation" checked={formData.a_corporation} onChange={handleChange} />S corporation
                  </div>
                  <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <p style={{ padding: '0 35px' }}>
                  {' '}
                  <bold className="bold">Note:</bold> Check the “LLC” box above and, in the entry space, enter the appropriate code (C, S, or P) for
                  the tax classification of the LLC, unless it is a disregarded entity. A disregarded entity should instead check the appropriate box
              ...
              <div style={{ display: 'flex', flexDirection: 'column', borderBottom: '1px solid black' }}>
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <div>
                    {' '}
                    If on line 3a you checked “Partnership” or “Trust/estate,” or checked “LLC” and entered “P” as its tax classification, and you are
                    providing this form to a partnership, trust, or estate in which you have an ownership interest, check this box if you have any
                    foreign partners, owners, or beneficiaries. See instructions &nbsp;&nbsp;&nbsp; . &nbsp;&nbsp;&nbsp; . &nbsp;&nbsp;&nbsp; .
                    &nbsp;&nbsp;&nbsp; . &nbsp;&nbsp;&nbsp; . &nbsp;&nbsp;&nbsp; . &nbsp;&nbsp;&nbsp; . &nbsp;&nbsp;&nbsp; . &nbsp;&nbsp;&nbsp; .{' '}
                    <input type="checkbox" name="federal_tax_b" checked={formData.federal_tax_b} onChange={handleChange} />
                  </div>
                </div>
              </div>
            </div>
            <div
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <div>Exemptions (codes apply only to certain entities, not individuals; see instructions on page 3):</div>
              </div>
              <span>
  ...
  <div></div> */}
      ...
      <div style={{ display: 'flex' }}>
        <div>
          <p>
            ...
            this is generally your social security number (SSN). However, for a resident alien, sole proprietor, or disregarded entity, see the
            instructions for Part I, later. For other entities, it is your employer identification number (EIN). If you do not have a number, see How
            to get a TIN, later.
          </p>
          <p>
            <span className="bold">Note:</span> If the account is in more than one name, see the instructions for line 1. See also What Name and
      ...
      <div className="row" style={{ padding: '0' }}>
        <div className="col-md-12" style={{ padding: '0', fontSize: '15px', border: '2px solid black', borderColor: 'black' }}>
          <span
            style={{
              fontWeight: '900',
              color: 'white ',
      ...
      <div style={{ display: 'flex' }}>
        <div>
          ...
          <p>
            <span className="bold">1:</span> The number shown on this form is my correct taxpayer identification number (or I am waiting for a number
            to be issued to me); and
          </p>
          <p>
            <span className="bold">2:</span> I am not subject to backup withholding because (a) I am exempt from backup withholding, or (b) I have not
            been notified by the Internal Revenue Service (IRS) that I am subject to backup withholding as a result of a failure to report all
          ...
          <p>
            <span className="bold">4:</span> The FATCA code(s) entered on this form (if any) indicating that I am exempt from FATCA reporting is
            correct.
          </p>
          <p>
            <span className="bold">Certification instructions:</span> TYou must cross out item 2 above if you have been notified by the IRS that you
            are currently subject to backup withholding because you have failed to report all interest and dividends on your tax return. For real
            estate transactions, item 2 does not apply. For mortgage interest paid, acquisition or abandonment of secured property, cancellation of
            debt, contributions to an individual retirement arrangement (IRA), and, generally, payments other than interest and dividends, you are not
      ...
      <div
        ...
        <div className="col-9">
          ...
          />
          {SignatureDataURL && <button onClick={handleDelete}>Delete</button>}
        </div>
        <p>
          Date:{' '}
          <input
